...
fuel_desc = e_sample["fuelTypeDescription"].unique().tolist()

sect_desc = e_sample["sectorDescription"].unique().tolist()
...
cost_selection = selection[selection["sectorDescription"] == "Electric Utility"]
cost = cost_selection["cost-per-btu"]
...
data = {
    "date": dates.to_list(),
    "gen (1e3 MwH)": total.to_list(),
    "rel_cost_btu": cost.to_list()
}
